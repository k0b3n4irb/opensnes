#!/usr/bin/env python3
"""65816 Assembly Cycle Counter for OpenSNES and PVSnesLib compiler output.

Analyzes .asm files generated by cc65816 (cproc + QBE w65816 backend)
or 816-tcc/816-opt (PVSnesLib) and estimates CPU cycle costs per function
based on the 65816 instruction timing reference.
...
Usage:
    ...
    python3 cyclecount.py --pvsneslib file.asm            # Force PVSnesLib format
...
# ---------------------------------------------------------------------------
# PVSnesLib format detection and support
# ---------------------------------------------------------------------------

def detect_pvsneslib(lines):
    """Detect if an assembly file was generated by PVSnesLib (816-tcc/816-opt).

    Heuristics: presence of .define __*_locals, .ifgr directives, or
    tcc__r0 direct page references (.b suffix).
    """
    for line in lines[:50]:
        stripped = line.strip()
        if re.match(r'\.define\s+__\w+_locals\s+\d+', stripped):
            return True
        if stripped.startswith('.ifgr '):
            return True
    return False


def parse_pvsneslib_defines(lines):
    """Parse .define __funcname_locals N directives.

    Returns dict mapping function name -> locals size (int).
    """
    defines = {}
    for line in lines:
        m = re.match(r'\.define\s+__(\w+)_locals\s+(\d+)', line.strip())
        if m:
...
def analyze_file(filename, branches_taken=False, verbose=False, pvsneslib=False):
    """Analyze an assembly file and return per-function cycle counts.

    Args:
        filename: path to .asm file
        branches_taken: count branches as taken (+1 cycle)
        verbose: collect per-instruction detail
        pvsneslib: force PVSnesLib format (auto-detected if False)
    ...
    # Auto-detect PVSnesLib format
    is_pvs = pvsneslib or detect_pvsneslib(lines)

    # For PVSnesLib: parse locals sizes and track .ifgr skip state
    pvs_locals = {}
    ifgr_skip = False  # True when inside .ifgr block that evaluates to false
    if is_pvs:
        pvs_locals = parse_pvsneslib_defines(lines)

    for line in lines:
        parsed = parse_line(line)

        # PVSnesLib: handle .ifgr / .endif conditional assembly
        if is_pvs and parsed['type'] == 'directive':
            raw = parsed['raw']
            raw_upper = raw.upper()

            # .ifgr __funcname_locals 0 — skip if locals == 0
            ifgr_match = re.match(r'\.ifgr\s+__(\w+)_locals\s+(\d+)', raw, re.IGNORECASE)
            if ifgr_match:
                func_name = ifgr_match.group(1)
                threshold = int(ifgr_match.group(2))
                locals_size = pvs_locals.get(func_name, 0)
                ifgr_skip = not (locals_size > threshold)
                continue

            if raw_upper.startswith('.ENDIF'):
                ifgr_skip = False
                continue

            # .ENDS marks section end
            if raw_upper.startswith('.ENDS'):
                current_func = None
                ifgr_skip = False
            continue

        # Skip instructions inside false .ifgr blocks
        if ifgr_skip:
            continue
        ...
        if parsed['type'] == 'label' and parsed['global']:
            ...
            # PVSnesLib: __local_N labels are internal to the parent function
            if is_pvs and name.startswith('__local_'):
                continue
        ...
        if not is_pvs and parsed['type'] == 'directive':
            raw = parsed['raw']
            # .ENDS marks section end
    ...
    if args.compare:
        ...
        for filename in args.files:
            functions = analyze_file(filename, args.branches_taken,
                                     args.verbose, pvsneslib=args.pvsneslib)
            if args.function:
                functions = OrderedDict(
