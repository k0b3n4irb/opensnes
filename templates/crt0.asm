...
.ifdef SA1
    ...
    ; bit=1 writable, $FF = fully writable. Resolved 2026-09-02 —
    ; fullsnes, the Nintendo dev manual (book 2 §4.1.25) and nocash
    ; (nesdev p=237542) agree; Mesen2/snes9x/luna behave accordingly.
    ; The Super Famicom Dev Wiki page states the OPPOSITE and is wrong
    ; — do not "fix" this to $00 (it write-blocks I-RAM and fails the
    ; self-test below with sa1_status=$FF).
...
-   dex
    ...
    ; Zero-fill the far C band ($7E:2000-$FFFF, 56 KB) — chantier B2.
    ; __far objects (`.RAMSECTION ".far.N" BANK $7E SLOT 2`) get the same
    ; zero-initialised guarantee as bank-0 statics. One fixed-source DMA
    ...
    stz.w $2183         ; WMADDH = 0    → bank $7E
    lda #$08
    sta.w $4300         ; DMAP0: CPU→PPU, fixed A-address, 1 byte/transfer
    lda #$80
    sta.w $4301         ; BBAD0: $2180 (WMDATA)
    ldx #ZeroByte
    stx.w $4302         ; A1T0 = address of a zero byte in ROM
    lda #:ZeroByte
    sta.w $4304         ; A1B0 = its bank
    ldx #$E000
    stx.w $4305         ; DAS0 = 56 KB
    lda #$01
    sta.w $420B         ; MDMAEN: go (blocks until done)
...
.ifdef SA1
    ...
    ; 1. Enable I-RAM writes (redundant with early init, but safe)
    ;    bit=1 = write-enable, $FF = fully writable — see the SIWP note
    ;    at the early-init site above.
    lda #$FF
...
; Fixed DMA source for the bank-$7E zero-fill above (chantier B2).
ZeroByte:
    .db 0
...
;   [target_addr:2][target_bank:1][size:2][data:N]
;
; The bank byte (chantier B2) is $00 for the C RAM band and $7E for
; __far objects (QBE `.RAMSECTION ".far.N" BANK $7E SLOT 2`); the copy
; writes through a 24-bit pointer so one loop serves both. The source
; stays 16-bit: .data_init is SEMIFREE in bank $00 (DBR = 0 here).
;
...
@init_loop:
    ...
    sta tcc__r2         ; tcc__r2 = RAM target address (low 16)

    ; Read target bank (1 byte at +2) into tcc__r2+2 = the 24-bit
    ; destination's bank byte (tcc__r2h, unused by this routine otherwise)
    ldy #2
    sep #$20
    .ACCU 8
    lda (tcc__r0),y
    ...
    ; Read size (2 bytes at +3)
    ldy #3
    lda (tcc__r0),y
    beq @next_record    ; If size == 0, skip to next (shouldn't happen)
    sta tcc__r4         ; tcc__r4 = size

    ; Source is at tcc__r0 + 5 (after addr, bank and size fields)
    lda tcc__r0
    clc
    adc #5
    sta tcc__r1         ; tcc__r1 = ROM source address

    ; Copy loop: copy tcc__r4 bytes from (tcc__r1) to [tcc__r2] (24-bit)
...
@copy_byte:
    lda (tcc__r1),y
    sta [tcc__r2],y     ; indirect long: bank from tcc__r2+2
    iny
    cpy tcc__r4
    bne @copy_byte
...
@next_record:
    ; Move to next init record: tcc__r0 += 5 + size
    lda tcc__r0
    clc
    adc #5              ; Skip addr, bank and size fields
