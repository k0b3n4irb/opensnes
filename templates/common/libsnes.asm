...
;------------------------------------------------------------------------------
; spc_wait_ready - Wait for SPC700 to be ready
; Waits until APUIO0 reads $AA and APUIO1 reads $BB
;------------------------------------------------------------------------------
spc_wait_ready:
    php
    sep #$20
...
;------------------------------------------------------------------------------
; spc_upload - Upload data to SPC700 RAM
; void spc_upload(u16 addr, const u8 *data, u16 size)
; Stack: [P:1] [retaddr:3] [size:2] [data:3] [addr:2]
;   addr at offset 10, data at offset 7, size at offset 5
;------------------------------------------------------------------------------
spc_upload:
    php
    rep #$30            ; 16-bit A/X/Y
    ...
    ; Get parameters
    lda 10,s            ; addr - destination in SPC RAM
    sta APUIO2          ; Send address to APUIO2/3

    lda 5,s             ; size
    tay                 ; Y = bytes remaining

    ; Get data pointer into tcc__r0
    lda 7,s             ; data low word
    sta tcc__r0
    sep #$20
    lda 9,s             ; data bank
    sta tcc__r0h
    ...
    ; Start transfer - send $CC to APUIO1, first byte to APUIO0, $00 to APUIO0
    sep #$20
    lda #$CC
    sta APUIO1

    ; Wait for SPC to acknowledge with APUIO0 = $CC
-   lda APUIO0
    cmp #$CC
    bne -
...
-   lda APUIO0
    ...
    rep #$20
    ldx #1              ; byte counter (already sent byte 0)
@upload_loop:
    cpy #0
    beq @upload_done
    dey
...
;------------------------------------------------------------------------------
; spc_execute - Start execution at address in SPC700
; void spc_execute(u16 addr)
; Stack: [P:1] [retaddr:3] [addr:2]
;   addr at offset 5
;------------------------------------------------------------------------------
spc_execute:
    php
    ...
    ; Send address to start execution
    lda 5,s             ; addr low
    sta APUIO2
    lda 6,s             ; addr high
    sta APUIO3

    ; Send execute command (non-zero, non-$CC to APUIO1)
    ; First get current counter
    lda APUIO0
    inc a
    inc a               ; +2 to ensure different from $CC path
    sta APUIO0

    ; Send execute flag
    lda #$01            ; Non-zero = execute (not continue upload)
    sta APUIO1
