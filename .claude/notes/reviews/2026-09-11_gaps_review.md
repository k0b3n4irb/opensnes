...
| R5 | ROM coverage: executed-symbol set, ratchet on never-executed lib symbols | G22, the 96 uncalled | M | **H** | wiring for symbol level (cycles > 0 in `profile --out` JSON); PC-set — **accepted by luna (2026-09-12) as `luna profile --pc-set out.bin`, "next in line"**; no prototype needed | commit the never-executed list as the ratchet |
| R6 | Audio regression by WAV hash | G21 | S | M | wiring (`--audio-out`); hash only | three examples: `audio/apu_switch`, one snesmod, one SFX |
| R7 | Manifests for the 36 interactive examples; multitap and mouse probes | `ScanMPlay5`, `ReadMouse`, 24 examples with no manifest | L | M | wiring for pad 1; port 2 **confirmed missing and accepted by luna (2026-09-12): `input2` in manifests and CLI**; `input/two_players` is the acceptance case | start with `input/move_sprite`, `input/two_players`, then the 7 `games/` |
