...
| G19 | `--power-on random[=seed]` (on `run`, `state`, `frames`, `diff`, `profile`) never used. Every run boots from zero-filled RAM. This is the bug class of the v0.40.0 and v0.41.1 boot fixes, which luna could not see | present on the CLI; absent from the `luna test` manifest schema | wiring + request (manifest key) |
...
|---|---|---|---|---|---|---|
| R1 | Corpus liveness + fbhash under `--power-on random=<seed>` | G19 | S | **H** | wiring (`run` / `state`); **request**: `power_on` key in `luna test` | a runner option that passes `--power-on random=1` (fixed seed) through to luna for coverage and compare; the WRAM oracle stays excluded (it hashes the stack by design) |
