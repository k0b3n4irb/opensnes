...
| R4 | `luna profile` replaces the static cyclecount; VBlank *time* budget | G17 G23 | M | M | **done 2026-09-17** — the blocker dissolved: `luna profile --sym` takes an explicit symbol file, so handing luna a copy of the `.sym` with the handler's 18 child labels stripped makes `NmiHandler` one row again (7258 mclk worst frame on `sprite_swarm`, against 652 for the entry stub alone). `tools/luna-test/nmi_budget.py` gates six representative examples at **12 000 master cycles** per frame — NTSC VBlank is ~51 800, and the corpus worst measured 8112 (`games/breakout`) — inside `make tests`, plus `make test-nmi-budget` on its own. The luna fold request is **withdrawn**, not pending: see the 2026-09-17 report |
...
| R7 | Manifests for the interactive examples; multitap and mouse probes | `ScanMPlay5`, `ReadMouse`, examples with no manifest | L | M | **done 2026-09-17 — 108 manifests, 85 of 85 examples covered** (was 51 and 35). The last one landed when luna v1.24.0 shipped `[asserts.ppu]`; the multitap stays out because our own path is unreachable code, all green. The first slice (2026-09-15) gave the 7 games and `input/move_sprite` input-driven tests; this pass covered the other 42 by asserting what each example *demonstrates* rather than that it runs: exact VRAM/CGRAM blocks for the loaders, A/B manifest pairs on the S-DSP for the four audio effects, library WRAM shadows for the HDMA/window/Mode 7 register state, OAM decoded through OBJSEL for the sprite sizes, and a negative wherever one was cheap. Every value was measured on the machine and the manifests were mutation-tested, not merely run. **Mouse sensitivity is now covered** (`mouse_sensitivity.toml`: luna models the whole request/cycle/report round trip, 0 → 1 → 2 → 0, and port 1 must not move). **`color/gradient_9bit` was the last gap** and is now covered: its whole effect is HDMA rewriting the backdrop and INIDISP per scanline, so the only frame-boundary observable is a PPU register — `inidisp = 5`, the brightness the last line of the table leaves, with `tm`/`ts`/`cgwsel` at 0 as the negatives. **The multitap cannot be tested, and cannot even run**: `snes_mplay5` is never written by anything, so `ScanMPlay5` is unreachable — now a 🟡 entry in `KNOWN_LIMITATIONS.md`, and luna models no multitap device either |
