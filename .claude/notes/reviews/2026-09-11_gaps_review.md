...
| R4 | `luna profile` replaces the static cyclecount; VBlank *time* budget | G17 G23 | M | M | wiring for per-symbol totals; per-frame max — **request accepted by luna (2026-09-12, `per_frame` + `--budget`), queued behind SA-1 work** | cross-check `devtools/cyclecount/bench.py` against `luna profile devtools/benchrom/benchrom.sfc --from-frame 2 --out -` |
| R5 | ROM coverage: executed-symbol set, ratchet on never-executed lib symbols | G22, the 96 uncalled | M | **H** | wiring for symbol level (cycles > 0 in `profile --out` JSON); PC-set — **shipped in luna v1.21.0 as `luna profile --pc-set out.bin`** (sorted u32 PCs, whole 24-bit space; fold FastROM mirrors on our side); wiring only | commit the never-executed list as the ratchet |
| R6 | Audio regression by WAV hash | G21 | S | M | wiring (`--audio-out`); hash only | three examples: `audio/apu_switch`, one snesmod, one SFX |
| R7 | Manifests for the 36 interactive examples; multitap and mouse probes | `ScanMPlay5`, `ReadMouse`, 24 examples with no manifest | L | M | wiring for pad 1; port 2 **shipped in luna v1.21.0: `input2` in manifests (top level and per checkpoint) and `luna state --input2`**; `input/two_players` is the acceptance case | start with `input/move_sprite`, `input/two_players`, then the 7 `games/` |
...
| R10 | Power-on-state dependencies found by R1 | R1 first run | S (audio ×6, **done 2026-09-12**) / M (scanline 223) | **H** | wiring done; findings are ROM/lib bugs | the six audio examples now call `dmaClearVRAM()` after `consoleInit()` (crt0 enables BG1 in TM and consoleInit points its tilemap at VRAM $0400 without clearing VRAM). **Open lib question for the owner:** should `consoleInit()` clear VRAM by default (one 64 KB DMA in forced blank at boot, PHILOSOPHY "sane defaults") so every minimal ROM is hardware-safe? The scanline-223 difference on `backgrounds/mode0`, `color/hicolor_blend`, `games/tetris` still needs a luna `state` peek of the last row from both power-on modes to attribute it |
